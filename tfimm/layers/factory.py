...
def act_layer_factory(act_layer: str):
    ...
    if act_layer in {"linear", "swish", "relu", "gelu", "sigmoid"}:
        return lambda **kwargs: tf_keras.layers.Activation(act_layer, **kwargs)
    if act_layer == "relu6":
        return lambda **kwargs: tf_keras.layers.ReLU(max_value=6, **kwargs)
    else:
...
def norm_layer_factory(norm_layer: str):
    ...
    if norm_layer == "":
        return lambda **kwargs: tf_keras.layers.Activation("linear", **kwargs)

    elif norm_layer == "batch_norm":
        bn_class = tf_keras.layers.BatchNormalization
        bn_args = {
        ...
        bn_class = tf_keras.layers.LayerNormalization
